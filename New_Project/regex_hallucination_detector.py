...
class RegexHallucinationDetector:
    ...
    def _check_suspicious_attribute(
        self, node: ast.Attribute, code: str, lines: List[str]
    ):
        """Check for suspicious attribute access"""
        attr_name = node.attr
        ...
        if any(
            prefix in attr_name for prefix in ["auto_", "smart_", "enhanced_", "magic_"]
        ):
            self._add_ast_finding(
    ...
    def generate_report(
        self, findings: List[SuspiciousPattern], filename: str
    ) -> Dict[str, Any]:
