...
class EnhancedNeo4jCodeAnalyzer:
    ...
    def __init__(self, hex_mode: bool = False):
        ...
        self.knowledge_graph_validator = (
            None  # Will be initialized with Neo4j connection
        )
    ...
    async def analyze_ai_generated_script(
        self, script_path: str, repo_name: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.knowledge_graph_validator:
                validation_result = (
                    await self.knowledge_graph_validator.validate_analysis(
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if isinstance(node.target, ast.Name):
                    constants.append(
                        {
                            "name": node.target.id,
                            "type": self._get_name(node.annotation)
                            if node.annotation
                            else "Any",
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id.endswith("Type"):
                        type_aliases.append(
                            {
                                "name": target.id,
                                "definition": self._get_name(node.value)
    ...
    def _extract_function_info(
        self, node: ast.FunctionDef, is_stub: bool, is_method: bool = False
    ) -> Dict[str, Any]:
    ...
    def _is_likely_internal(self, import_name: str, project_modules: Set[str]) -> bool:
        ...
        return base_module not in self._external_modules and any(
            import_name.startswith(pm) for pm in project_modules
        )
...
class EnhancedKnowledgeGraphExtractor:
    ...
    async def setup_enhanced_constraints(self):
        ...
        async with self.driver.session() as session:
            ...
            try:
                ...
                await session.run(
                    "DROP CONSTRAINT constraint_3c126057 IF EXISTS"
                )  # Old Method constraint
                await session.run(
                    "DROP CONSTRAINT constraint_fa941a93 IF EXISTS"
                )  # Old Function constraint
    ...
    async def get_python_files(self, repo_path: str) -> List[Path]:
        ...
        for root, dirs, files in os.walk(repo_path):
            ...
            for file in files:
                ...
                if (
                    file.endswith(".py") or file.endswith(".pyi")
    ...
    async def analyze_repository(self, repo_url: str, temp_dir: str = None):
        ...
        try:
            ...
            logger.info(
                f"Analyzed {len(implementation_files)} implementation files and {len(stub_files)} stub files"
            )

            # Create enhanced graph with type validation relationships
            await self._create_enhanced_graph(
                repo_name, modules_data, stub_files, implementation_files
            )

            # Create type validation relationships
            await self._create_type_validation_relationships(
                repo_name, stub_files, implementation_files
            )
    ...
    def _get_project_modules(
        self, python_files: List[Path], repo_root: Path
    ) -> Set[str]:
    ...
    def _calculate_layer_distribution(self, modules_data: List[Dict]) -> Dict[str, int]:
        ...
        for mod in modules_data:
            if self.hex_mode and self.analyzer.hex_analyzer:
                analyzed_mod = (
                    self.analyzer.hex_analyzer.analyze_component_architecture(mod)
                )
                if "architectural_info" in analyzed_mod:
                    layer = analyzed_mod["architectural_info"].get(
    ...
    async def _create_enhanced_graph(
        self,
        repo_name: str,
        ...
        async with self.driver.session() as session:
            ...
            codebase_props = {"name": repo_name, "has_stubs": len(stub_files) > 0}
            ...
            for mod in modules_data:
                ...
                if self.hex_mode and self.analyzer.hex_analyzer:
                    ...
                    logger.info(
                        f"🏗️  Architectural analysis completed for: {canonical_name}"
                    )

                logger.info(
                    f"📄 Creating file node: {canonical_name} (has_stub: {has_stub})"
                )
                ...
                if self.hex_mode and "architectural_info" in analyzed_mod:
                    ...
                    file_props.update(
                        {
                            "dominant_layer": arch_info.get(
                                "dominant_layer", "unknown"
                            ),
                            "architectural_types": arch_info.get(
                                "architectural_types", []
                            ),
                            ...
                            "cyclomatic_complexity": file_metrics[
                ...
                props_str = ", ".join(
                    [f"{k}: ${k}" for k in file_props.keys() if k != "created_at"]
                )
                ...
                if self.hex_mode and "architectural_info" in analyzed_mod:
                    dominant_layer = analyzed_mod["architectural_info"].get(
                        "dominant_layer", "unknown"
                    )
                ...
                for cls in analyzed_mod["classes"]:
                    ...
                    class_props = {
                        ...
                        "stub_docstring": stub_class.get("docstring", "")
                        if stub_class
                        else "",
                    }
                    ...
                    if self.hex_mode and "architectural_tags" in cls:
                        ...
                        class_props.update(
                            {
                                "architectural_type": arch_tags.get(
                                    "architectural_type", "unknown"
                                ),
                                "layer": arch_tags.get("layer", "unknown"),
                                "hexagonal_role": arch_tags.get(
                                    "hexagonal_role", "unknown"
                                ),
                                ...
                                "public_method_count": class_metrics[
                                    "public_method_count"
                                ],
                    ...
                    set_props = ", ".join(
                        [
                            f"c.{k} = ${k}"
                            for k in class_props.keys()
                            if k not in ["file_path", "name", "module"]
                        ]
                    )
                    ...
                    for method in cls["methods"]:
                        ...
                        """,
                            ...
                            stub_signature=stub_method.get("signature", "")
                            if stub_method
                            else "",
                            stub_return_type=stub_method.get("return_type", "")
                            if stub_method
                            else "",
                        )
                ...
                for func in mod["functions"]:
                    ...
                    """,
                        ...
                        stub_signature=stub_function.get("signature", "")
                        if stub_function
                        else "",
                        stub_return_type=stub_function.get("return_type", "")
                        if stub_function
                        else "",
                    )
            ...
            logger.info(
                f"✅ Knowledge graph created with {len(stub_files)} stub overlays"
            )

    async def _create_type_validation_relationships(
        self, repo_name: str, stub_files: List[Dict], implementation_files: List[Dict]
    ):
    ...
    async def validate_function_signature(
        self, module_name: str, function_name: str, class_name: str = None
    ) -> Dict[str, Any]:
    ...
    async def analyze_ai_script_with_validation(
        self, script_path: str, repo_name: str = None
    ) -> Dict[str, Any]:
        """Analyze AI-generated script using integrated knowledge graph validation"""
        return await self.analyzer.analyze_ai_generated_script(script_path, repo_name)

    async def validate_ai_code_snippet(
        ...
        try:
            ...
            try:
                ...
                result = await self.analyzer.analyze_ai_generated_script(
                    temp_file_path, context.get("repo_name") if context else None
                )
                return result
            finally:
    ...
    async def _create_architectural_relationships(
        self, repo_name: str, modules_data: List[Dict[str, Any]]
    ):
            ...
            for class_info in classes_with_arch:
                ...
                for base_class in class_info["base_classes"]:
                    try:
                        ...
                        logger.warning(
                            f"Could not create inheritance relationship {class_name} -> {base_class}: {e}"
                        )
                ...
                if dominant_layer != "unknown":
                    ...
                    for other_class in classes_with_arch:
                        if other_class["class_name"] != class_name:
                            other_layer = other_class["architectural_metadata"].get(
                                "layer", "unknown"
                            )

                            # Create DEPENDS_ON relationships based on architectural patterns
                            if self._should_create_dependency_relationship(
                                dominant_layer, other_layer
                            ):
                                try:
                                    await session.run(
                                        """
                                    ...
                                    logger.warning(
                                        f"Could not create dependency relationship: {e}"
                                    )

        logger.info(f"✅ Created {relationships_created} architectural relationships")

    def _should_create_dependency_relationship(
        self, source_layer: str, target_layer: str
    ) -> bool:
...
async def setup_enhanced_knowledge_graph(codebase_path: str, hex_mode: bool = False):
    ...
    extractor = EnhancedKnowledgeGraphExtractor(
        neo4j_uri, neo4j_user, neo4j_password, hex_mode=hex_mode
    )
    ...
    try:
        ...
        async with extractor.driver.session() as session:
            result = await session.run(
                "MATCH (c:Codebase) RETURN c.name as name, c.has_stubs as has_stubs"
            )
            existing_repos = [
                (record["name"], record.get("has_stubs", False))
        ...
        if (
            os.path.exists(repo_input)
            or repo_input.startswith("./")
            or os.path.isabs(repo_input)
        ):
            ...
            print(
                "📋 Processing both .py and .pyi files for comprehensive type analysis..."
            )
...
"""
                    ...
                    print(
                        "   Use extractor.detect_hallucination(code, context) with actual data"
                    )
        ...
        if result["stub_files"] > 0:
            ...
            print(
                "   Use extractor.validate_function_signature(module, function, class) with actual data"
            )
...
async def main():
    ...
    try:
        await setup_enhanced_knowledge_graph(
            codebase_path=args.codebase_path, hex_mode=args.hex
        )
        ...
        if args.hex:
            print(
                "🏗️  Architectural metadata has been integrated into the knowledge graph"
            )
    except Exception as e:
        print(f"\n❌ Setup failed: {e}")
        return 1
