...
class HexagonalArchitectureAnalyzer:
    ...
    def _analyze_file_architecture(self, module_data: Dict) -> Dict:
        ...
        for arch_type, patterns in self.domain_patterns.items():
            for pattern in patterns:
                if pattern in file_lower or any(
                    pattern in part.lower() for part in path_parts
                ):
        ...
        return {
            ...
            "confidence": max(layer_scores.values()) / 5.0
            if max(layer_scores.values()) > 0
            else 0.1,
        }

    def _analyze_class_architecture(
        self, class_data: Dict, module_data: Dict
    ) -> ArchitecturalTag:
        ...
        arch_type = (
            max(arch_scores, key=arch_scores.get)
            if max(arch_scores.values()) > 0
            else "unknown"
        )
        ...
        reasoning = self._generate_reasoning(
            class_name, arch_type, layer, hex_role, max_score
        )
    ...
    def _analyze_function_architecture(
        self, function_data: Dict, module_data: Dict
    ) -> Dict:
        ...
        primary_context = (
            max(context_scores, key=context_scores.get)
        ...
        return {
            ...
            "confidence": max(context_scores.values()) / 3.0
            if max(context_scores.values()) > 0
            else 0.1,
        }
    ...
    def _is_interface_class(self, class_data: Dict) -> bool:
        ...
        class_name = class_data.get("name", "")
        if (
            class_name.startswith("I") and class_name[1:2].isupper()
        ):  # Interface naming convention
            return True

        if any(
            pattern in class_name.lower()
            for pattern in ["interface", "protocol", "contract"]
        ):
            return True
    ...
    def _generate_reasoning(
        self, class_name: str, arch_type: str, layer: str, hex_role: str, score: int
    ) -> str:
