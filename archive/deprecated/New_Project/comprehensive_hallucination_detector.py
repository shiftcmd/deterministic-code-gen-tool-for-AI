...
class ComprehensiveHallucinationDetector:
    ...
    def __init__(
        ...
        self.kg_detector = AIHallucinationDetector(
    ...
    async def analyze_file(
        ...
        context = {"script_path": file_path, "module_context": Path(file_path).stem}
        framework_result = await self.framework_detector.detect_hallucinations(
            code, context
        )
        ...
        combined_result = self._combine_results(
            file_path, kg_result, framework_result, regex_findings
        )

        # 5. Save Combined Report
        if output_dir:
            await self._save_comprehensive_report(
                combined_result, file_path, output_dir
            )
    ...
    def _combine_results(
        ...
        kg_confidence = kg_result.get("validation_summary", {}).get(
        ...
        overall_confidence = (
            kg_confidence * 0.5
            + framework_confidence * 0.3  # Knowledge graph gets highest weight
            + max(0, 1.0 - (len(regex_findings) * 0.1))  # Framework analysis
            * 0.2  # Regex findings (penalty-based)
        )

        # Determine if manual review is needed
        needs_manual_review = (
            manual_review_count > 0
            or overall_confidence < 0.8
            or critical_patterns > 0
            or not kg_valid
            or not framework_valid
        )
    ...
    def _generate_recommendations(
        ...
        if regex_findings:
            recommendations.append(
                f"Found {len(regex_findings)} suspicious patterns requiring review"
            )

            critical_findings = [
                f for f in regex_findings if f.suspicion_level.value == "critical"
            ]
            if critical_findings:
                recommendations.append(
                    f"URGENT: {len(critical_findings)} critical patterns found - review immediately"
                )
            ...
            for finding in regex_findings[:3]:  # Top 3 findings
                if finding.suggestions:
                    recommendations.append(
                        f"Line {finding.line_number}: {finding.suggestions[0]}"
                    )

        # Confidence-based recommendations
        if confidence < 0.7:
            recommendations.append(
                "Low confidence score - recommend thorough manual review"
            )
        elif confidence < 0.9:
            recommendations.append(
                "Moderate confidence - spot-check suspicious patterns"
            )

        return recommendations

    async def _save_comprehensive_report(
        self, result: ComprehensiveResult, file_path: str, output_dir: str
    ):
    ...
    def _generate_markdown_report(self, result: ComprehensiveResult) -> str:
        ...
        if result.needs_manual_review:
            ...
            manual_items = [
                f for f in result.regex_findings if f.manual_review_required
            ]
    ...
    def print_summary(self, result: ComprehensiveResult):
        ...
        print(
            f"  🔍 Regex: {len(result.regex_findings)} patterns ({result.critical_patterns} critical)"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive Hallucination Detection"
    )
