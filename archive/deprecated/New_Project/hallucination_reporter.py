...
class HallucinationReporter:
    ...
    def _generate_markdown_content(self, report: Dict[str, Any]) -> str:
        ...
        if total_validations > 0:
            md.append(
                f"- **Valid:** {summary['valid_count']} ({summary['valid_count']/total_validations:.1%})"
            )
            md.append(
                f"- **Invalid:** {summary['invalid_count']} ({summary['invalid_count']/total_validations:.1%})"
            )
            md.append(
                f"- **Not Found:** {summary['not_found_count']} ({summary['not_found_count']/total_validations:.1%})"
            )
            md.append(
                f"- **Uncertain:** {summary['uncertain_count']} ({summary['uncertain_count']/total_validations:.1%})"
            )
        else:
            md.append(f"- **Valid:** {summary['valid_count']} (0.0%)")
            md.append(f"- **Invalid:** {summary['invalid_count']} (0.0%)")
            md.append(f"- **Not Found:** {summary['not_found_count']} (0.0%)")
            md.append(f"- **Uncertain:** {summary['uncertain_count']} (0.0%)")
