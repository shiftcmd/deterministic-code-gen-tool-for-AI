...
def debug_parsing(repo_path: str = "."):
    ...
    for i, file_path in enumerate(python_files[:10]):  # Test first 10 files
        ...
        try:
            module_data = analyzer.analyze_python_file(
                file_path, repo_root, project_modules
            )
    ...
    if test_file.exists():
        try:
            ...
            module_data = analyzer.analyze_python_file(
                test_file, repo_root, project_modules
            )
            if module_data:
                classes = module_data.get("classes", [])
                functions = module_data.get("functions", [])
                ...
                if len(classes) != len(classes_found):
                    print(
                        f"   ⚠️  MISMATCH: AST found {len(classes_found)} classes, analyzer found {len(classes)}"
                    )

                if len(functions) != len(functions_found):
                    print(
                        f"   ⚠️  MISMATCH: AST found {len(functions_found)} functions, analyzer found {len(functions)}"
                    )
